    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Our Garden</h1>
        ...
        <Tabs.Root defaultValue={plants[0].name} className="w-full">
          <Tabs.List className="flex flex-wrap gap-2 border-b border-gray-200 mb-8">
            {plants.map((plant) => (
              <Tabs.Trigger
                key={plant.name}
                value={plant.name}
                className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 hover:border-gray-300 border-b-2 border-transparent data-[state=active]:border-green-500 data-[state=active]:text-green-600 transition-all"
              >
                {plant.name}
              </Tabs.Trigger>
            ))}
          ...
          {plants.map((plant) => (
            <Tabs.Content key={plant.name} value={plant.name}>
              <div className="bg-white rounded-lg shadow-lg p-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-2">{plant.name}</h2>
                <p className="text-sm italic text-gray-500 mb-6">{plant.scientificName}</p>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">{plant.care}</p>
                  </div>

                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Blooming Season</h3>
                    <p className="text-gray-700">{plant.season}</p>
                  </div>
                </div>
              </div>
            </Tabs.Content>
        ...
        <div className="mt-12 bg-blue-50 rounded-lg p-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-3">Garden Care Tips</h3>
